...
const RELEVANT_FILES = ['models.json', 'auth-profiles.json', 'SOUL.md', 'RULES.md', 'TOOLS.md'];
  ...
  try {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isFile() && RELEVANT_FILES.includes(entry.name)) {
      ...
      return NextResponse.json({ agentId, agentDir, files });
    } catch (err: any) {
      if (err.code === 'ENOENT') {
        return NextResponse.json({ agentId, agentDir, files: [], warning: `Agent directory does not exist: ${agentDir}` });
      }
      throw err;
    }
  } catch (error: any) {
    console.error('Failed to list agent files:', error);
    return NextResponse.json({ error: error.message || 'Failed to list agent files' }, { status: 500 });
  }
}
