  ...
  try {
    ...
    if (!action) {
      return NextResponse.json(
        { error: "action is required (add, update, remove)" },
        { status: 400 }
      );
    }
    ...
    if (action === "add") {
      if (!agentData || !agentData.name) {
        return NextResponse.json(
          { error: "agentData with name is required for add" },
          { status: 400 }
        );
      }
      ...
      if (!agentId) {
        return NextResponse.json(
          { error: "agentId is required for remove" },
          { status: 400 }
        );
      }
    ...
    // Write updated config (pretty-printed)
